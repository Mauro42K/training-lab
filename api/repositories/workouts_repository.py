...
def upsert_workouts(
    ...
    *,
    ...
    normalized = {item.healthkit_workout_uuid: item for item in workouts}
    ...
    existing_stmt = select(Workout.healthkit_workout_uuid).where(
        ...
        Workout.healthkit_workout_uuid.in_(uuids),
    )
    ...
    updated = len(existing_uuids)
    inserted = len(unique_workouts) - updated
