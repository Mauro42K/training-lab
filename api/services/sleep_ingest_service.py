from dataclasses import dataclass

from sqlalchemy.orm import Session

from api.core.config import get_settings
from api.repositories.daily_domains_repository import (
    SleepSessionUpsert,
    get_sleep_session_snapshots_by_uuids,
    upsert_sleep_sessions,
)
from api.repositories.idempotency_repository import compute_request_hash, create_record, get_record
from api.repositories.user_repository import get_or_create_default_user, update_user_timezone_if_valid
from api.schemas.ingest import IngestSleepSessionsResponse, SleepSessionsIngestRequest
from api.services.daily_domain_rules import APPLE_HEALTH_PROVIDER
from api.services.ingest_service import IdempotencyConflictError, PayloadLimitExceededError
from api.services.local_date import resolve_authoritative_timezone_name, resolve_sleep_session_local_date
from api.services.sleep_recompute_service import SleepRecomputeService


@dataclass
class SleepIngestService:
    db: Session

    def ingest_sleep_sessions(
        self,
        *,
        payload: SleepSessionsIngestRequest,
        idempotency_key: str,
    ) -> IngestSleepSessionsResponse:
        settings = get_settings()
        if len(payload.sleep_sessions) > settings.ingest_max_batch_size:
            raise PayloadLimitExceededError(
                f"Batch size {len(payload.sleep_sessions)} exceeds INGEST_MAX_BATCH_SIZE={settings.ingest_max_batch_size}"
            )

        user = get_or_create_default_user(self.db)
        payload_dict = payload.model_dump(mode="json")
        request_hash = compute_request_hash(payload_dict)

        existing = get_record(self.db, user.id, idempotency_key)
        if existing is not None:
            if existing.request_hash != request_hash:
                raise IdempotencyConflictError(
                    "Idempotency key already used with a different payload"
                )
            replay_response = dict(existing.response_json)
            replay_response["idempotent_replay"] = True
            return IngestSleepSessionsResponse.model_validate(replay_response)

        authoritative_timezone = resolve_authoritative_timezone_name(
            request_timezone=payload.timezone,
            stored_timezone=user.timezone,
            fallback_timezone=settings.trimp_timezone_fallback,
        )
        update_user_timezone_if_valid(
            self.db,
            user=user,
            timezone_name=authoritative_timezone,
        )

        sleep_uuids = [item.healthkit_sleep_uuid for item in payload.sleep_sessions]
        pre_snapshots = get_sleep_session_snapshots_by_uuids(
            self.db,
            user_id=user.id,
            sleep_uuids=sleep_uuids,
        )
        inserted, updated = upsert_sleep_sessions(
            self.db,
            rows=[
                SleepSessionUpsert(
                    healthkit_sleep_uuid=item.healthkit_sleep_uuid,
                    user_id=user.id,
                    start_at=item.start,
                    end_at=item.end,
                    local_date=resolve_sleep_session_local_date(
                        start_at=item.start,
                        end_at=item.end,
                        user_timezone=authoritative_timezone,
                        fallback_timezone=settings.trimp_timezone_fallback,
                    ),
                    category_value=item.category_value,
                    source_bundle_id=item.source_bundle_id,
                    provider=APPLE_HEALTH_PROVIDER,
                    source_count=item.source_count,
                    has_mixed_sources=item.has_mixed_sources,
                    primary_device_name=item.primary_device_name,
                )
                for item in payload.sleep_sessions
            ],
        )

        recompute_summary = SleepRecomputeService(settings=settings).recompute_for_sleep_uuids(
            self.db,
            user_id=user.id,
            user_timezone=authoritative_timezone,
            sleep_uuids=sleep_uuids,
            pre_snapshots=pre_snapshots,
        )

        response = IngestSleepSessionsResponse(
            inserted=inserted,
            updated=updated,
            total_received=len(payload.sleep_sessions),
            rebuilt_dates=recompute_summary.rebuilt_dates,
            invalidated_daily_recovery_dates=recompute_summary.invalidated_daily_recovery_dates,
            idempotent_replay=False,
        )

        create_record(
            self.db,
            user_id=user.id,
            idempotency_key=idempotency_key,
            request_hash=request_hash,
            response_json=response.model_dump(mode="json"),
            status_code=200,
        )
        self.db.commit()
        return response
